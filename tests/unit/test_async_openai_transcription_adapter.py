"""
Unit tests for AsyncOpenAITranscriptionAdapter.

Tests use mocked AsyncOpenAI client.
"""
...
class TestAsyncOpenAITranscriptionAdapter:
    """Tests for AsyncOpenAITranscriptionAdapter."""

    def test_adapter_name_is_openai(self):
        adapter = AsyncOpenAITranscriptionAdapter(api_key="test-key")
        assert adapter.name == "openai"

    def test_default_model_is_whisper_1(self):
        adapter = AsyncOpenAITranscriptionAdapter(api_key="test-key")
        assert adapter._default_model == "whisper-1"

    def test_custom_default_model(self):
        adapter = AsyncOpenAITranscriptionAdapter(
            api_key="test-key", default_model="gpt-4o-transcribe"
        )
        assert adapter._default_model == "gpt-4o-transcribe"
    ...
    def test_client_created_without_base_url(self):
        adapter = AsyncOpenAITranscriptionAdapter(api_key="test-key")
    ...
    async def test_prompt_passed_to_sdk(self):
        ...
        await adapter.transcribe(b"audio", prompt="Meeting notes about AI")

        call_kwargs = mock_client.audio.transcriptions.create.call_args.kwargs
        assert call_kwargs["prompt"] == "Meeting notes about AI"
    ...
    async def test_custom_model_override(self):
        ...
        await adapter.transcribe(b"audio", model="gpt-4o-transcribe")
